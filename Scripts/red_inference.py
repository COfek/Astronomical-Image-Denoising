...
def red_restore(y: torch.Tensor, model: torch.nn.Module, kernel: torch.Tensor,lambda_: float = 0.05,
                alpha: float = 0.1, max_iter: int = 30, verbose: bool = True, device: str = "cuda" if torch.cuda.is_available() else "cpu") -> torch.Tensor:
    """
    Perform Regularization by Denoising (RED) image restoration.

    Args:
        y (torch.Tensor): Noisy observed image (shape: [1, 1, H, W]).
        model (torch.nn.Module): Pretrained denoising model (e.g., UNet).
    ...
    for i in range(max_iter):
        ...
        if verbose and (i % 5 == 0 or i == max_iter - 1):
            ...
            ssim = compute_ssim_batch(x.detach(), y.detach())
